...
namespace ComplaintManagementSystem.Controllers
{
    ...
    public class EscalationController : ControllerBase
    {
        ...
        {
            var employees = await _escalationService.GetEligibleEmployeesAsync(complaintId, action);
            return Ok(employees);
        }
        ...
        {
            var result = await _escalationService.GetNextEscalationLevelAsync(complaintId);
            return Ok(result);
        }
    }
}
